...
def process_csv(file_path, log_widget, preview_widget):
    try:
        # Tentando detectar automaticamente o delimitador
        try:
            df_original = pd.read_csv(file_path, delimiter=None)
        except pd.errors.ParserError:
            # Se falhar, tentar com diferentes delimitadores comuns
            delimiters = [',', ';', '\t', '|']
            for delimiter in delimiters:
                try:
                    df_original = pd.read_csv(file_path, delimiter=delimiter)
                    break
                except pd.errors.ParserError:
                    continue
            else:
                messagebox.showerror("Erro", f"Não foi possível ler o arquivo CSV com os delimitadores esperados.")
                return
        ...
        # Exibir o CSV original no preview
        ...
        col_lat = simpledialog.askinteger("Coluna Latitude", "Em qual coluna está a latitude (1 a n)?", minvalue=1, maxvalue=100)
        col_lon = simpledialog.askinteger("Coluna Longitude", "Em qual coluna está a longitude (1 a n)?", minvalue=1, maxvalue=100)
        col_date = simpledialog.askinteger("Coluna Date", "Em qual coluna está a data (1 a n)?", minvalue=1, maxvalue=100)
        col_time = simpledialog.askinteger("Coluna Time", "Em qual coluna está o tempo (1 a n)?", minvalue=1, maxvalue=100)
        ...
        # Perguntar ao usuário os novos cabeçalhos para o CSV
        new_lat_header = 'latitude'
        new_lon_header = 'longitude'
        new_date_header = 'date'
        new_time_header = 'time'
        ...
        if "datetime" in mapped_columns:
            ...
            df["datetime"] = pd.to_datetime(df[mapped_columns["date"]].astype(str) + " " + df[mapped_columns["time"]].astype(str), errors="coerce")
        else:
        ...
        # Exibir o conteúdo do CSV processado no preview
        preview_widget.configure(state='normal')
        preview_widget.delete(1.0, tk.END)
        preview_widget.insert(tk.END, df.to_string(index=False))
        preview_widget.configure(state='disabled')

        # Salvar o arquivo com o nome correto
        save_confirmation = messagebox.askyesno("Salvar", "Deseja salvar o CSV processado?")
